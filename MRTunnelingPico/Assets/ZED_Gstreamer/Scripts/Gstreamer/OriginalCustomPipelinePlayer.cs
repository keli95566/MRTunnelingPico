...
public class OriginalCustomPipelinePlayer : MonoBehaviour {
	...
	void Start () {
		...
		_img.Create (1, 1, GstImageInfo.EPixelFormat.EPixel_I420);

		BlittedImage = new Texture2D (1280, 720);
	...
	void OnFrameBlitted(GstBaseTexture src,int index)
	{
		...
		_newFrame = true;
		if (_newFrame) {
			m_Texture.Player.BlitTexture(BlittedImage.GetNativeTexturePtr(),1280,720);
			//BlitToTexture (BlittedImage);
			_newFrame = false;
		}
